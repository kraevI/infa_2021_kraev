...
def sun():
    ...
    for i, j, k in [[305, 130, 15], [292, 260, 10], [308, 0, 10], [165, 122, 10], [430, 138, 10]]:
        circle(surface2, (250, 250, 215, 255), (i, j), k)

def bear_fisherman(x, y, s):
    """ x,y - bear's position, s - its size """
    if s < 0.5: i = 1
    else: i = 4
    lines(surface2, BLACK, False, [[x + 151*s, y + 125*s], [x + 181*s, y + 60*s], [x + 220*s, y + 20*s], [x + 280*s, y - 25*s], [x + 340*s, y - 45*s]], i)
    for c, i, j, k, l in [[ICE_COLOR, 0, 0, 150, 260], [ICE_COLOR, 77, -35, 98, 55], [ICE_COLOR, 117, 60, 70, 30], [ICE_COLOR, 72, 185, 110, 80], [ICE_COLOR, 122, 235, 95, 35], [GRAY, 252, 160, 160, 60], [DARK_BLUE, 264, 172, 136, 48]]:
        ...
        ellipse(surface2, BLACK, (x + i*s, y + j*s, k*s, l*s), 1)
    line(surface2, BLACK, [x + 340*s, y - 45*s], [x + 340*s, y + 205*s], 1)
